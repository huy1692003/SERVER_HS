...
namespace API_HomeStay_HUB.Services
{
    public class BookingHandler : IBookingHandler
    {
        ...
        public BookingHandler(
            ...
            ISendMaillService sendMaillService
        )
        {
        ...
        public async Task ProcessBookingAsync(Booking booking)
        {
            var errorStr = await _bookingRepository.createBooking(booking);
            if (errorStr)
            {
        ...
        private async Task SendEmailToHomestayOwner(Booking booking, string EmailOwner)
        {
            try
            {
                ...
                // Tạo tiêu đề email
                string titleSendMail = $"Thông báo: Đơn đặt phòng cho Homestay {homestay!.HomeStay!.HomestayName}";
                ...
                // Gửi email
                _=_sendMaillService.SendMaill(EmailOwner, titleSendMail, content);
